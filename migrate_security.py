...
with app.app_context():
    ...
    try:
        with db.engine.connect() as conn:
            conn.execute(text("ALTER TABLE user_sessions ADD COLUMN jti VARCHAR(64) UNIQUE"))
            conn.execute(text("ALTER TABLE user_sessions ADD COLUMN last_used DATETIME"))
            conn.execute(text("ALTER TABLE user_sessions ADD COLUMN user_agent TEXT"))
            conn.execute(text("ALTER TABLE user_sessions ADD COLUMN ip_address VARCHAR(45)"))
            conn.execute(text("ALTER TABLE user_sessions ADD COLUMN is_active BOOLEAN DEFAULT 1"))
            conn.commit()
