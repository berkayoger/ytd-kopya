...
def check_env_security() -> list[str]:
    ...
    if not jwt_secret_match:
        ...
        if (
            jwt_secret
            == "your-jwt-secret-key-at-least-64-characters-long-for-production"
        ):
    ...
    if (
        "REDIS_PASSWORD=" not in env_content
        or "REDIS_PASSWORD=your-redis-password" in env_content
    ):
...
def check_dependencies() -> List[str]:
    ...
    for dep in security_deps:
        if dep not in requirements:
            issues.append(
                f"⚠️  Security dependency '{dep}' not found in requirements.txt"
            )
