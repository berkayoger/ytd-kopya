...
def client(monkeypatch):
    ...
    monkeypatch.setattr(
        "backend.middleware.plan_limits.enforce_plan_limit",
    ...
    monkeypatch.setattr(
        "backend.api.batch._dispatch",
        lambda symbols, timeframe, asset: type("J", (), {"id": "job1"}),
    )
    monkeypatch.setattr(
        "backend.api.batch.celery_app.AsyncResult", lambda job_id: FakeRes(job_id)
    )
...
def _submit(c, symbols, tf="1h", asset="crypto", headers=None):
    return c.post(
        "/api/batch/submit",
        json={"symbols": symbols, "timeframe": tf, "asset": asset},
        headers=headers or {},
    )
