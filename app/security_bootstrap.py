...
def _apply_secure_defaults(app):
    ...
    # Rate-limit anahtarı string/boolean karışıklığını önlemek için normalize et
    rle = app.config.get("RATE_LIMIT_ENABLED", os.getenv("RATE_LIMIT_ENABLED", "true"))
    try:
        app.config["RATE_LIMIT_ENABLED"] = (str(rle).lower() == "true")
    except Exception:
...
def _setup_rate_limit(app):
    # İstenirse tamamen devre dışı
    if app.config.get("RATE_LIMIT_ENABLED") is False:
        app.logger.info("Rate limiting disabled by config")
        return None
...
def bootstrap_security(app):
    ...
    if login_rl and app.config.get("RATE_LIMIT_ENABLED", True):
