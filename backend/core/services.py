...
class DataCollector:
    ...
    def collect_price_data(self, coin: str) -> Dict[str, Any]:
        ...
        try:
            ...
            times = [datetime.fromtimestamp(p[0] / 1000).isoformat() for p in data['prices']]
            ...
            # 5 dakikalık Redis önbelleği
            if self.redis:
                self.redis.set(cache_key, json.dumps(result), ex=300)
    ...
    def _calc_indicators(self, prices: List[float]) -> Dict[str, float]:
        ...
        macd = float(ta.macd(series)['MACD_12_26_9'].iloc[-1]) if len(series) > 26 else 0.0
        bb = ta.bbands(series)
        stochastic = float(ta.stoch(series)['STOCHk_14_3_3'].iloc[-1]) if len(series) > 14 else 50.0
...
class YTDCryptoSystem:
    ...
    def analyze(self, coin: str, profile: str, user: Optional[User] = None) -> Dict[str, Any]:
    ...
    def backtest_rules(self, coin: str, profile: str, start: str, end: str) -> Dict[str, Any]:
        # Geriye dönük test için DBHData’dan al
        records = DBHData.query.filter(
            DBHData.coin == coin,
            DBHData.timestamp >= start,
            DBHData.timestamp <= end
        ).all()
