...
# Mevcut modellerle uyumlu kalmak adına importları fonksiyon içine taşıyacağız.
...
# ---- Back-compat alias ------------------------------------------------------
# Bazı eski testler/importlar PLAN_LIMITS ismini bekler. Yeni PLAN_DEFAULTS
# sözlüğünün birebir kopyası olacak şekilde alias oluşturuyoruz.
...
def _normalize_plan_name(user) -> str:
    ...
    - String, Enum veya Plan nesnesinden gelen değerleri işler.
    - None veya bilinmeyen değerlerde 'BASIC' döner.
    """
    raw = None
    try:
...
def get_user_effective_limits(arg1: Any = None,
    ...
    """
    # Yeni tarz: (user_id, feature_key) — keyword/positional destekle
...
def enforce_plan_limits(limit_key: str) -> Callable:
    ...
    def decorator(fn: Callable) -> Callable:
        def inner(*args, **kwargs):
            # Local importlar: import döngüsünü ve toplama sırasındaki sorunları azaltır
            from backend.db.models import User, UsageLog, db  # type: ignore
            # Kullanıcıyı çöz
