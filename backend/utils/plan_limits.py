...
def check_custom_feature(user, feature: str) -> bool:
    """Kullanıcı/plan veya **global feature flag** üzerinden özelliği doğrula."""
    # 1) Uygulama genel feature-flag (testler create_feature_flag ile burayı set ediyor)
    try:
        flags = (current_app and current_app.config.get("FEATURE_FLAGS")) or {}
        if isinstance(flags, dict) and flags.get(feature) is True:
            return True
    except Exception:
        pass
