...
from backend.db.models import AlarmSeverityEnum, SecurityAlarmLog
...
def send_alarm(
    ...
    if not webhook:
        logger.warning(
            "SLACK_ALARM_WEBHOOK_URL tanımlı değil, Slack bildirimi atlanıyor."
        )
    ...
    if socketio:
        try:
            socketio.emit(
                "alert",
                {"type": alert_type, "severity": severity.value, "details": details},
                namespace="/alerts",
            )
        except Exception as e:
