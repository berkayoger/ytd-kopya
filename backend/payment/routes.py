...
from iyzipay.models import (Address, BasketItem, Buyer, CheckoutFormInitialize,
                            Payment)
from loguru import logger

from backend import limiter
# Modelleri import et
from backend.db.models import (PaymentTransactionLog, PromoCode,
                               PromoCodeUsage, SubscriptionPlan, User, db)
...
from backend.utils.security import (check_iyzico_signature,
                                    verify_iyzico_signature)
...
)  # Ödeme başlatmak için en az TRIAL planı gereklidir
def initiate_payment():
    ...
    try:
        ...
        with current_app.app_context():
            ...
            final_price = current_app.config["BACKEND_PLAN_PRICES"].get(
                selected_plan.name
            )
            if final_price is None:
                logger.error(
                    f"Plan {selected_plan.name} için backend'de fiyat tanımlı değil."
                )
                add_audit_log(
                    action_type="PAYMENT_INIT_MISSING_PRICE_CONFIG",
                    actor_id=user.id,
                    actor_username=user.username,
                    target_username=selected_plan.name,
                    details={"ip_address": request.remote_addr},
                    ip_address=request.remote_addr,
                    commit=True,
                )
                return (
                    jsonify({"error": "Seçilen plan için fiyat bilgisi bulunamadı."}),
                    500,
                )
            ...
            if promo_code_str:
                ...
                if promo_code_obj and promo_code_obj.plan.value == selected_plan.value:
                    ...
                    logger.info(
                        f"Promosyon kodu '{promo_code_str}' ödeme başlatma aşamasında kullanılıyor."
                    )
                    add_audit_log(
                        action_type="PAYMENT_INIT_PROMO_CODE_PRESENT",
                        actor_id=user.id,
                        actor_username=user.username,
                        target_id=promo_code_obj.id,
                        target_username=promo_code_obj.code,
                        details={
                    ...
                    logger.warning(
                        f"Kullanıcı {user.username} tarafından geçersiz/aktif olmayan promosyon kodu denemesi: {promo_code_str} (ödeme başlatma)"
                    )
                    add_audit_log(
                        ...
                        details={
                            "reason": "Invalid or expired promo code during payment init",
                            "ip_address": request.remote_addr,
                        },
                        ip_address=request.remote_addr,
                        commit=False,
            ...
            request_iyzico.price = str(
                final_price
            )  # iyzico'ya gönderilen fiyat (backend'den doğrulanmış)
            request_iyzico.paid_price = str(final_price)  # Kullanıcının ödeyeceği fiyat
            request_iyzico.currency = (
                Payment.CURRENCY_USD
            )  # Para birimi (USD olarak ayarlı, iyzico'ya göre ayarla)
            ...
            request_iyzico.callback_url = (
                f"{callback_base_url.rstrip('/')}/api/payment/callback"
            )

            request_iyzico.basket_id = f"YTD-PLAN-{selected_plan.name}-{user.id}-{request_iyzico.conversation_id}"  # Sepet ID'si
            request_iyzico.payment_group = (
                IyzipayConstants.PAYMENT_GROUP_SUBSCRIPTION
            )  # Abonelik ödemesi
            ...
            buyer.identity_number = (
                "11111111111"  # Placeholder, gerçek veriden alınmalı (TCKN vb.)
            )
            buyer.last_login_date = user.created_at.strftime(
                "%Y-%m-%d %H:%M:%S"
            )  # Son login
            buyer.registration_date = user.created_at.strftime(
                "%Y-%m-%d %H:%M:%S"
            )  # Kayıt tarihi
            ...
            # iyzico API çağrısı
            checkout_form_initialize = CheckoutFormInitialize.create(
                request_iyzico, options
            )
            ...
            if iyzico_result.get("status") == "success":
                logger.info(
                    f"iyzico ödeme başlatma başarılı. ConversationID: {request_iyzico.conversation_id}, Kullanıcı: {user.username}"
                )
                add_audit_log(
                ...
                return jsonify(
                    {
                        ...
                        "token": iyzico_result.get(
                            "token"
                        ),  # Gerekliyse token'ı da döndür
                    }
                )
            else:
                error_message = iyzico_result.get(
                    "errorMessage", "Bilinmeyen iyzico hatası."
                )
                error_code = iyzico_result.get("errorCode", "N/A")
                logger.error(
                    f"iyzico ödeme başlatma hatası. Konuşma ID: {request_iyzico.conversation_id}, Hata: {error_message}, Kod: {error_code}. Kullanıcı: {user.username}"
                )
                add_audit_log(
...
def iyzico_callback():
    ...
    if not iyzico_token or not conversation_id:
        ...
        add_audit_log(
            ...
            details={
                "reason": "Missing token or conversationId",
                "data": data,
                "ip_address": request.remote_addr,
            },
            ip_address=request.remote_addr,
    ...
    try:
        ...
        if (
            iyzico_details.get("status") == "success"
            and iyzico_details.get("paymentStatus") == "SUCCESS"
        ):
            # Ödeme başarılı!
            payment_id = iyzico_details.get("paymentId")
            # 3) Replay saldırı önleme
            if PaymentTransactionLog.query.filter_by(
            ...
            with current_app.app_context():  # DB işlemleri için app context
                ...
                if not user:
                    logger.error(
                        f"iyzico callback: Kullanıcı bulunamadı. User ID: {user_id_from_basket}"
                    )
                    add_audit_log(
                        action_type="PAYMENT_CALLBACK_FAILED_USER_NOT_FOUND",
                        actor_username="iyzico_callback_system",
                        target_id=user_id_from_basket,
                        details={
                            "data": iyzico_details,
                            "ip_address": request.remote_addr,
                        },
                ...
                try:
                    ...
                    logger.error(
                        f"iyzico callback: Geçersiz plan adı. Plan: {plan_name_from_basket}"
                    )
                    add_audit_log(
                        ...
                        target_username=user.username,
                        details={
                            "plan_name": plan_name_from_basket,
                            "data": iyzico_details,
                            "ip_address": request.remote_addr,
                        },
                        ip_address=request.remote_addr,
                ...
                expected_price = current_app.config["BACKEND_PLAN_PRICES"].get(
                    selected_plan.name
                )

                if (
                    not expected_price or abs(total_paid_price - expected_price) > 0.01
                ):  # Küsürat farkı için tolerans
                    logger.critical(
                        f"iyzico callback: Fiyat doğrulama hatası! Ödenen: {total_paid_price}, Beklenen: {expected_price}. Kullanıcı: {user.username}"
                    )
                    add_audit_log(
                ...
                if user.subscription_end and user.subscription_end > datetime.utcnow():
                    user.subscription_end += timedelta(
                        days=30
                    )  # iyzico varsayılan olarak 30 günlük periyot sağlar.
                else:
                    user.subscription_end = datetime.utcnow() + timedelta(days=30)

                # Maksimum abonelik süresi kontrolü (ör: 5 yıl)
                MAX_SUBSCRIPTION_EXTENSION_DAYS = current_app.config.get(
                    "MAX_SUBSCRIPTION_EXTENSION_DAYS", 5 * 365
                )
                ...
                logger.info(
                    f"iyzico callback: Ödeme başarılı. Kullanıcı {user.username} planı {selected_plan.name} olarak güncellendi."
                )
                add_audit_log(
            ...
            iyzico_details.get("status") == "success"
            and iyzico_details.get("paymentStatus") == "FAILURE"
        ):
            # Ödeme başarısız
            error_message = iyzico_details.get("errorMessage", "Ödeme başarısız.")
            logger.warning(
                f"iyzico callback: Ödeme başarısız. ConversationID: {conversation_id}, Hata: {error_message}. Data: {iyzico_details}"
            )
            add_audit_log(
            ...
            logger.error(
                f"iyzico callback: Bilinmeyen ödeme durumu. ConversationID: {conversation_id}. Data: {iyzico_details}"
            )
            add_audit_log(
