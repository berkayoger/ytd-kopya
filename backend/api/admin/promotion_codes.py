...
from backend.db.models import PromotionCode, db

admin_promotion_bp = Blueprint(
    "admin_promotion", __name__, url_prefix="/api/admin/promo"
)
...
def list_promos():
    ...
    if filter_val:
        ilike = f"%{filter_val}%"
        q = q.filter(
            (PromotionCode.code.ilike(ilike)) | (PromotionCode.description.ilike(ilike))
        )
...
def create_promo():
    ...
    promo = PromotionCode(
        ...
        custom_users=(
            ",".join(data.get("custom_users", []))
            if isinstance(data.get("custom_users"), list)
            else data.get("custom_users")
        ),
    )
