...
from backend.utils.security import (
    validate_symbols_list, validate_timeframe, validate_asset,
    safe_cache_key, ip_allowed, is_2fa_required, is_user_2fa_ok,
    need_admin_approval, has_admin_approval
)
...
def submit_batch():
    ...
    if not feature_flag_enabled("draks") or not os.getenv("DRAKS_BATCH_ENABLED", "false").lower() in {"1","true","yes","on"}:
    ...
    try:
        r = current_app.extensions["redis_client"]
        is_anom = record_submit(r, str(getattr(user, "id", "")) if user else None, request.remote_addr or "unknown")
        if is_anom:
...
def _dispatch(symbols: List[str], timeframe: str, asset: str):
    ...
    return run_batch.apply_async(kwargs={"symbols": symbols, "timeframe": timeframe, "asset": asset}, time_limit=timeout, soft_time_limit=timeout-5)
    ...
    for s in symbols:
        ...
        try:
            ...
            if rss_mb > mem_limit_mb:
                raise MemoryError(f"Worker memory over limit: {rss_mb:.1f}MB > {mem_limit_mb}MB")
        except Exception:
            pass
